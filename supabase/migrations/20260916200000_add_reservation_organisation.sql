-- Add direct organisation ownership to reservations while preserving the
-- existing reservation transaction contract and atomic workflow.

alter table public.reservations
    add column organisation_id uuid
        references public.organisations(id);

create index reservations_organisation_id_idx
    on public.reservations (organisation_id);
...
begin
    if exists (
        ...
        raise exception
            'Cannot backfill reservation organisation ownership: referenced customer organisation is missing';
    end if;
...
begin
    ...
    into v_customer_id, v_reservation_id, v_reservation_organisation_id
    from public.reservations as r
