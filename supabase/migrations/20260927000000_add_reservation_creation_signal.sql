-- 3D3-E0: expose an authoritative new-vs-replay signal to trusted callers.
...
declare
    ...
    v_customer_organisation_id uuid;
...
begin
    ...
    v_operational_start := p_operational_start at time zone 'Europe/Paris';
    v_operational_end := p_operational_end at time zone 'Europe/Paris';
    ...
    select r.customer_id, r.id, r.organisation_id, r.status
    into v_customer_id, v_reservation_id,
         v_reservation_organisation_id, v_reservation_status
    ...
    if v_reservation_id is not null then
        if exists (
            ...
            where r.id = v_reservation_id
              and not (
                  r.product_id is not distinct from p_product_id
                  ...
                  and r.weekly_price_at_booking is not distinct from p_weekly_price_at_booking
        ...
        select c.organisation_id into v_customer_organisation_id
        from public.customers as c where c.id = v_customer_id;

        if v_reservation_organisation_id is null
           or v_customer_organisation_id is null
           or v_reservation_organisation_id <> v_customer_organisation_id then
            raise exception 'reservation/customer organisation ownership mismatch for idempotent retry';
        end if;

        select j.id into v_delivery_job_id from public.service_jobs as j
        where j.reservation_id = v_reservation_id and j.job_type = 'delivery'
        order by j.created_at limit 1;
        select j.id into v_collection_job_id from public.service_jobs as j
        where j.reservation_id = v_reservation_id and j.job_type = 'collection'
        order by j.created_at limit 1;
        select a.id, a.machine_id, a.status, a.hold_expires_at
        into v_allocation_id, v_machine_id, v_allocation_status, v_hold_expires_at
        from public.allocations as a
        where a.reservation_id = v_reservation_id
          and a.status in ('held', 'reserved', 'active')
        order by a.created_at limit 1;

        if v_reservation_status <> 'pending' or v_allocation_status <> 'held' then
        ...
        return query select v_customer_id, v_reservation_id, v_delivery_job_id,
            ...
            v_reservation_status, v_hold_expires_at, false;
        return;
    end if;

    select o.id into v_organisation_id from public.organisations as o
    where o.slug = 'igloue';
    ...
    if (select count(*) from public.reservations as r
        ...
        where r.organisation_id = v_organisation_id
          ...
          and exists (select 1 from public.allocations as a
                      where a.reservation_id = r.id and a.status = 'held')) >= 2 then
        raise exception 'Too many active holds' using errcode = 'P1001';
    end if;

    insert into public.customers (organisation_id, first_name, last_name, email, phone)
    values (v_organisation_id, p_first_name, p_last_name, p_email, nullif(p_phone, ''))
    ...
    insert into public.reservations (
        organisation_id, customer_id, product_id, quantity, rental_start, rental_end,
        ...
        v_customer_organisation_id, v_customer_id, p_product_id, 1,
        p_rental_start, p_rental_end, 'pending', p_delivery_address_line_1,
        nullif(p_delivery_address_line_2, ''), p_delivery_postcode, p_delivery_city,
        p_delivery_zone, p_weekly_price_at_booking, p_delivery_fee, p_options_total,
        p_deposit_amount, p_total_amount, 'not_started', trim(p_idempotency_key)
    ) returning id, status into v_reservation_id, v_reservation_status;
    ...
    select h.allocation_id, h.machine_id into v_allocation_id, v_machine_id
    from public.create_machine_hold(v_reservation_id, v_operational_start, v_operational_end) as h;
    ...
    from public.allocations as a where a.id = v_allocation_id;

    return query select v_customer_id, v_reservation_id, v_delivery_job_id,
        v_collection_job_id, v_allocation_id, v_machine_id,
        v_reservation_status, v_hold_expires_at, true;
end;
$$;
