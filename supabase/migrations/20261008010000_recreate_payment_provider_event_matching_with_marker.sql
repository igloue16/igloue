-- Extend P3D3 to durably mark successful authoritative matching.
-- No payment, reservation, allocation, hold, or outbox state changes occur.

create or replace function public.match_payment_provider_event(
...
begin
    ...
    -- Lock order remains payment attempt, then provider event. This preserves
    -- the reservation -> allocation -> attempt -> event suffix planned for P3E.
    ...
    if v_event.payment_attempt_id is not null then
        -- A legacy linked row without matched_at is not treated as proven
        -- P3D3 validation and is never upgraded by this migration.
        if v_event.matched_at is null then
            return query select
                'conflict'::text, null::uuid, null::uuid, null::uuid;
            return;
        end if;
