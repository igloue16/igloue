-- Align the availability RPC with the reservation transaction's
-- Europe/Paris civil-time authority.
--
-- The Edge Functions intentionally pass operational timestamps without an
-- offset.  Keep that representation at the RPC boundary and convert it
-- explicitly so the database session timezone cannot change availability.

drop function public.check_product_machine_availability(
    text,
    timestamptz,
    timestamptz
);

create function public.check_product_machine_availability(
    p_product_id text,
...
begin
    ...
    where pm.product_id = p_product_id
      ...
      and (pm.unavailable_until is null or pm.unavailable_until <= v_operational_start)
      and not exists (
          select 1
          ...
          where a.machine_id = pm.id
            ...
            and tstzrange(a.operational_start, a.operational_end, '[)')
                && tstzrange(v_operational_start, v_operational_end, '[)')
      );
end;
$$;

revoke all on function public.check_product_machine_availability(
    text,
...
grant execute on function public.check_product_machine_availability(
    text,
    timestamp without time zone,
    timestamp without time zone
) to service_role;
