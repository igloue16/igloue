-- Serialize and limit new pending holds per organisation/contact.
...
begin
    ...
    if v_reservation_id is not null then
        if exists (
            ...
            left join public.service_jobs as dj
              on dj.reservation_id = r.id
             and dj.job_type = 'delivery'
            left join public.service_jobs as cj
              on cj.reservation_id = r.id
             and cj.job_type = 'collection'
        ...
        into v_customer_organisation_id
        ...
        if v_reservation_organisation_id is null
           ...
           or v_reservation_organisation_id <> v_customer_organisation_id then
            raise exception
                'reservation/customer organisation ownership mismatch for idempotent retry';
        end if;
        ...
        if v_reservation_status <> 'pending'
           or v_allocation_status <> 'held' then
        ...
        select v_customer_id, v_reservation_id, v_delivery_job_id,
               ...
               v_reservation_status, v_hold_expires_at;
        return;
    end if;
    ...
    -- Same-contact new bookings serialize until commit/rollback. Under the
    -- normal READ COMMITTED RPC transaction the following statement sees
    -- the preceding lock holder's committed booking.
    perform pg_catalog.pg_advisory_xact_lock(
    ...
    if (
        ...
        raise exception 'Too many active holds'
            using errcode = 'P1001';
    end if;
