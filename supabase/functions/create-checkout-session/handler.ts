...
const MIN_REMAINING_MS = 11 * 60 * 1000;
...
export type CheckoutRpcClient = {
  rpc(name: string, parameters: Record<string, unknown>): Promise<{ data: unknown; error: RpcError }>;
};
...
function holdIsSafe(holdExpiresAt: unknown, now: Date): boolean {
  if (typeof holdExpiresAt !== "string") return false;
  const expiry = Date.parse(holdExpiresAt);
  return Number.isFinite(expiry) && expiry - now.getTime() >= MIN_REMAINING_MS;
}

function mapRpcError(error: RpcError) {
  if (!error) return null;
  if (error.code === "22023") return errorResponse(400, "INVALID_REQUEST");
  if (error.code === "P0001" || error.code === "P0002") return errorResponse(409, "PAYMENT_UNAVAILABLE");
  return errorResponse(503, "PAYMENT_UNAVAILABLE");
}

function mapStripeError(error: unknown, logError?: (...values: unknown[]) => void) {
  if (error instanceof StripeAdapterError) {
    if (error.kind === "timeout") return errorResponse(504, "PAYMENT_PROVIDER_TIMEOUT");
    if (error.kind === "provider_4xx") return errorResponse(502, "PAYMENT_PROVIDER_ERROR");
    if (error.kind === "provider_5xx") return errorResponse(502, "PAYMENT_PROVIDER_UNAVAILABLE");
  }
...
export async function handleCheckoutRequest(request: Request, dependencies: Dependencies) {
  ...
  if (!isObject(body) || !hasOnlyKeys(body, ["reservationId", "paymentCapability", "idempotencyKey"])) {
    return errorResponse(400, "INVALID_REQUEST");
  }
  ...
  const initiated = await dependencies.supabaseAdmin.rpc("initiate_reservation_payment", {
  ...
  if (!attempt || typeof attempt.payment_attempt_id !== "string" || typeof attempt.organisation_id !== "string") {
    dependencies.logError?.("payment initiation returned invalid data");
    return errorResponse(503, "PAYMENT_UNAVAILABLE");
  }

  const stateResult = await dependencies.supabaseAdmin.rpc("get_reservation_payment_checkout", {
  ...
  if (!state || typeof state.hold_expires_at !== "string" || typeof state.eligible !== "boolean") {
  ...
  if (state.provider_checkout_session_id || state.provider_checkout_url) {
    if (state.attempt_status !== "checkout_open" || !state.eligible ||
        typeof state.provider_checkout_session_id !== "string" || !isValidUrl(state.provider_checkout_url)) {
      return errorResponse(409, "PAYMENT_WINDOW_CLOSED");
    }
    return response({ ok: true, checkout: { url: state.provider_checkout_url, holdExpiresAt: state.hold_expires_at } }, 200);
  }

  const now = dependencies.now ?? new Date();
  if (!state.eligible || !holdIsSafe(state.hold_expires_at, now)) {
    return errorResponse(409, "PAYMENT_WINDOW_CLOSED");
  }
  const authoritativeAmount = typeof state.amount === "number"
    ? state.amount
    : typeof state.amount === "string" ? Number(state.amount) : NaN;
  if (!Number.isFinite(authoritativeAmount) || state.currency !== "EUR" ||
      typeof state.customer_email !== "string" || typeof state.reservation_id !== "string") {
    return errorResponse(503, "PAYMENT_UNAVAILABLE");
  }
  ...
  const persisted = await dependencies.supabaseAdmin.rpc("persist_reservation_payment_checkout", {
    p_payment_attempt_id: attempt.payment_attempt_id,
    p_organisation_id: attempt.organisation_id,
  ...
  if (!persistedRow || typeof persistedRow.hold_expires_at !== "string" ||
      typeof persistedRow.eligible !== "boolean") {
    dependencies.logError?.("checkout persistence returned invalid data");
    return errorResponse(503, "PAYMENT_UNAVAILABLE");
  }
  if (!persistedRow.eligible) return errorResponse(409, "PAYMENT_WINDOW_CLOSED");
  return response({ ok: true, checkout: { url: session.url, holdExpiresAt: persistedRow.hold_expires_at } }, 201);
}

export function createProductionDependencies(supabaseAdmin: CheckoutRpcClient): Dependencies {
